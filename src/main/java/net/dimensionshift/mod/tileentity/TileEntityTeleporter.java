...
public class TileEntityTeleporter extends TileEntity implements ISidedInventory {
	...
	public void updateEntity() {
		...
		if (active != this.activeLastTick) {
			BlockTeleporter.updateBlockType(active, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
		...
		if (playerNumber > 0 && this.slots[0] != null && this.slots[0].isItemEqual(new ItemStack(DimensionShift.itemDimensionIdentificationCrystal))) {
			...
			if (this.dimensionEnergy >= this.requiredDimensionEnergyForOnePerson * playerNumber && this.cooldown <= 0 && this.slots[0].stackTagCompound != null) {
				if (!this.getWorldObj().isRemote) {
					...
					List<Vector3d> list = GetClosestBlockInRadiusRelativeToStartPoint.getBlocks(dWorld, x, y, z, r, DimensionShift.blockTeleporterActive);
					...
					for (int i = list.size(); i > 0; i--) {
						...
						if (tileEntity2 != null) {
							TileEntityTeleporter teleporter2 = (TileEntityTeleporter) tileEntity2;
	...
	public String getInvName() {
		return this.isInvNameLocalized() ? this.localizedName : "container.teleporter";
	}
