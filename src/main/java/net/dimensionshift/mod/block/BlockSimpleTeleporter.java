...
public class BlockSimpleTeleporter extends BlockContainer {
	...
	public BlockSimpleTeleporter(boolean active) {
		super(Material.iron);
	...
	public static void updateBlockType(boolean active, World world, int x, int y, int z) {
		...
		if (active) {
			world.setBlock(x, y, z, DimensionShift.blockSimpleTeleporterActive);
		} else {
			world.setBlock(x, y, z, DimensionShift.blockSimpleTeleporterIdle);
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
		...
		if (world.isBlockIndirectlyGettingPowered(x, y, z) && MultiblockTeleporter.isMultiBlockStructure(world, x, y, z)) {
			...
			TileEntity tileEntity = world.getTileEntity(x, y, z);
			if (tileEntity != null) {
				TileEntitySimpleTeleporter teleporter = (TileEntitySimpleTeleporter) tileEntity;
	...
	public void breakBlock(World world, int x, int y, int z, Block oldBlock, int oldMetadata) {
		if (!keepInventory) {// if block is not broken by update status function
			TileEntitySimpleTeleporter tileentity = (TileEntitySimpleTeleporter) world.getTileEntity(x, y, z);
			if (tileentity != null) {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float hity, float hitz) {
		...
		if (tileEntity == null || player.isSneaking()) {
			...
			player.openGui(DimensionShift.instance, DimensionShift.guiIdSimpleTeleporter, world, x, y, z);
	...
	public TileEntity createNewTileEntity(World world) {
		return new TileEntitySimpleTeleporter();
	}
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack itemstack) {
		...
		if (itemstack.hasDisplayName()) {
			((TileEntitySimpleTeleporter) world.getTileEntity(x, y, z)).setGuiDisplayName(itemstack.getDisplayName());
		}
	...
	public Item getItem(World p_149694_1_, int p_149694_2_, int p_149694_3_, int p_149694_4_) {
		return Item.getItemFromBlock(DimensionShift.blockSimpleTeleporterIdle);
	}
